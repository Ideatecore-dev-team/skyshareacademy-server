const service = require("./service");

const create = async (req, res, next) => {
  try {
    let gambarSekolahPath;

    if (
      !req.files ||
      !req.files.gambar_sekolah ||
      req.files.gambar_sekolah.length === 0
    ) {
      gambarSekolahPath =
        "https://res.cloudinary.com/dsh5ppscb/image/upload/v1714931793/no-image/No_Image_Available_zfarlj.jpg";
    } else {
      gambarSekolahPath = req.files.gambar_sekolah[0].path;
    }

    const request = {
      nama_sekolah: req.body.nama_sekolah,
      gambar_sekolah: gambarSekolahPath,
      lokasi: req.body.lokasi,
      nama_group: req.body.nama_group,
    };

    const response = await service.create(request);
    res.status(201).json({
      data: response,
      status: "success",
      errors: false,
    });
  } catch (error) {
    next(error);
  }
};

const getAll = async (req, res, next) => {
  try {
    const response = await service.getAll();
    res.status(200).json({
      data: response,
      status: "success",
      errors: false,
    });
  } catch (error) {
    next(error);
  }
};

const getById = async (req, res, next) => {
  try {
    const request = {
      id: req.params.partnerId,
    };
    const response = await service.getById(request);
    res.status(200).json({
      data: response,
      status: "success",
      errors: false,
    });
  } catch (error) {
    next(error);
  }
};

const update = async (req, res, next) => {
  try {
    let gambarSekolahPath;

    if (
      !req.files ||
      !req.files.gambar_sekolah ||
      req.files.gambar_sekolah.length === 0
    ) {
      gambarSekolahPath =
        "https://res.cloudinary.com/dsh5ppscb/image/upload/v1714931793/no-image/No_Image_Available_zfarlj.jpg";
    } else {
      gambarSekolahPath = req.files.gambar_sekolah[0].path;
    }

    const request = {
      id: req.params.partnerId,
      nama_sekolah: req.body.nama_sekolah,
      gambar_sekolah: gambarSekolahPath,
      lokasi: req.body.lokasi,
      nama_group: req.body.nama_group,
    };
    const response = await service.update(request);
    res.status(200).json({
      data: response,
      status: "success",
      errors: false,
    });
  } catch (error) {
    next(error);
  }
};

const remove = async (req, res, next) => {
  try {
    const request = {
      id: req.params.partnerId,
    };
    const response = await service.remove(request);
    res.status(200).json({
      data: response,
      status: "success",
      errors: false,
    });
  } catch (error) {
    next(error);
  }
};

module.exports = {
  create,
  getAll,
  getById,
  update,
  remove,
};
